...
nextElement() : 받은 폼 값 중 다음 폼의 이름을 반환한다.
*/
Enumeration names = request.getParameterNames();
while (names.hasMoreElements()) {
	
	//폼의 name속성을 하나씩 읽어서 String 형으로 반환한다.
	String paramName = (String)names.nextElement();
	System.out.println("파라미터명 : " + paramName);
	
	if (paramName.equals("sex") ||
			paramName.equals("favorite")) {
		// checkbox와 radio는 getParameterValues()를 통해서 읽어온다.
		System.out.println("getParameterValues()로 읽기");
		out.println("파라미터명을 getParameterValue()로 "
				+ "얻어서 값 출력함<br/>");
	}
	else {
		//나머지 text와 textarea는s getParameter()로 읽어온다.
