...
<body>
	...
	엘리먼트를 통해 등록하게 된다. 이를 컨텍스트 초기화 파라미터 라고 한다.
	 --%>
	<h3>web.xml에 설정한 내용읽어오기</h3>
	<ul>
		<li>오라클드라이버 :
		<%=application.getInitParameter("JDBVDriver") %></li>
		<li>오라클접속URL :
	...
	얻어올수 있따. 사용시 context root를 제외한  /로 시작하는 절대경로를
	입력해야 한다.
	 --%>
	...
	<%--
	선언부(자바영역)에서 JSP의 내장 객체를 사용할떄는 해당 내장객체의
	클래스타입으로 사용해야 한다. application객체는
	ServletContext타입을 가지고 있따.
	 --%>
	...
	//2.매개변수로 내장객체를 전달하기
	/*
	Java영역에서는 JSP 의 내장객체를 바로사용하는것은 불가능하다.
