...
<body>
	<!-- 
	JSTL의 import태그
		-include액션태그와 같은 기능으로 reqeust영역을 공유한다.
		서로다른페이지이므로 page영역은 공유되지 않는다.
		-url속성에 절대경로를 설정시 컨텍스트 루프명은 제외한다.
	 -->
	<h2>import 태그</h2>
	<c:set var="requestVar" value="리퀘스트영역" scope="request" />
	
	<h3>var속성 미지정 - 해당위치에 바로 삽입된.(include 액션태그와
		동일함)</h3>
