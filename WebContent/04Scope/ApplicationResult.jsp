...
<body>
   <!--  
      ...
      그 영역이 공유되고 웹브라우저를 종료했다가 다시 실행하더라도 그 영역이 
      공유된다. 즉 한번 저장되면 웹서버가 셧다운(종료)될때 까지는 속성값이 
      유지된다.
   -->
   ...
   <!--  
      톰켓 웹서버를 완전히 종료(셧다운)항 후 해당 페이지를 단독으로 실행하게
      되면 Nullpointer 예외가 발생하게 된다.
      아래 코드는 application영역에 저장된 속성이 있다는 가정하에 속성을 
      읽어오게 되는데 저장된 속성이 없기 때문에 오류가 발생하게 된다.
   -->
   ...
   <h4>해당 페이지에서만 사용가능하고, 페이지이동 또는 forward되면
   해당 영역은 사라진다.</h4>
   <%
