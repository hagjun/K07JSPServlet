...
<%
/*  
application 영역에서 속성 저장하기
- 어플리케이션 영역에 저장된 속성은 모든 JSP(Servlet) 페이지에서 공유된다.
- 링크를 통해 새오운 페이지에 대한 요청이나 포워드, 인클루드 등 모든 페이지에서
   영역이 공유된다. 이런 페이지들은 하나의 웹어플리케이션 내에 존재하기 때문이다.
*/
//현재날짜 가져오기
java.util.Date myUtileDate = new java.util.Date();
System.out.println("myUtilDate="+myUtileDate);

java.sql.Date mySqlDate = new Date(myUtileDate.getTime());
...
//List계열의 컬렉션 생성후 DTO객체를 저장한다.(ArrayList와 사용법은 동일함)
